...
- §1–§15 — core language: `component`/`view` split, `param`/`prop`/`state`, control flow, `style`, constraints, `enum`, `env::*`/`once!`, reactive attribute expressions and `<=>`, i18n (Fluent), imports, an overview of the `UIElement` tree-exploration contract, and the full list of ~24 static verification rules (§14) a future compiler/linter must implement.
- 付録A — the `builtin::` namespace and `#[overrides(builtin::X)]` override rule (static verification only; the builtins themselves are in `docs/specs/builtins_spec.md` 付録F), plus the `#[embedded]`/`#[sealed]`/`#[native]`/`#[abstract]`/`#[content(field_name)]` component-level attributes.
