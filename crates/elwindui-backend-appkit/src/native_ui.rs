...
use crate::inner::{InnerButton, InnerMenu, InnerMenuBar, InnerMenuBarItem, InnerMenuItem, InnerTabView, InnerTextArea, InnerWindow, TabChipImpl};
use crate::AnyView;
...
/// over uniformly. `inherits = NativeControl` directly (an ordinary class, no `struct_only`/hand-
/// written intermediate trait) — `insert_tab`/`remove_tab`/`set_tab_content_visible` are plain
/// `InnerTabView` methods, not a separate cross-backend trait, since a real tab content host type
/// differs per backend (AppKit's `Retained<TreeHostView>`/`TabChipImpl` have no common shape with
/// WinUI3's own equivalents worth sharing without associated types this crate doesn't need yet).
#[elwindui_macros::class(inherits = crate::NativeControl)]
pub struct TabView {
    inner: InnerTabView,
...
impl TabView {
    ...
    #[inherent]
    fn rebuild(&self) {
        let this = self.weak_self.borrow().upgrade().expect("elwindui: TabView dropped while rebuilding");
