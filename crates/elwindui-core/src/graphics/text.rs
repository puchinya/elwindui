...
//! properties into a `TextFormatting` struct. See `docs/design/runtime/text_design.md` for the full
...
/// that don't exist anywhere in `elwindui-core` yet — see `docs/design/runtime/text_design.md`).
#[derive(Debug, Clone)]
pub struct TextMeasureRequest<'a> {
    ...
    /// (未対応, `docs/design/runtime/text_design.md`). Carried here so a backend that gains one has a
