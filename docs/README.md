...
**`specs/`は未実装の仕様を含む場合があるが規範仕様（「何であるべきか」）であり、`design/`はその実現方法を記述する内部設計書（「どう作るか」）である。** 現在の実際の動作や対応状況はソースコードおよび `status/` を参照すること。コードや `status/` と `specs/` に不一致があっても、それだけで仕様の誤りを意味しない。ドキュメント権限・判断ルールの詳細はルート [`AGENTS.md`](../AGENTS.md) を参照。
