...
`unmount_override` closes the Window's own active custom popup/context-menu surface (`TreeHostView`/`TreeHostPanel::close_active_popup`) *before* its own content unmounts, and clears the close-request handler *before* forwarding to the backend's own `unmount_override` — a stale handler must never remain reachable once the Window starts tearing down.
