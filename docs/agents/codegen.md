...
- **Option shape**: A Prop declared as `Option<T>` keeps `Option<T>` in storage, getter, setter, and `new!` initial assignment. Do not infer an inner-`T` setter or revive the former deferred-Option constructor rule.
