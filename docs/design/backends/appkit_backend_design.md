...
Application startup runs on the main thread. Window and tree hosts own AppKit views and schedule coalesced layout/render passes on that thread. An independently hosted subtree, such as TabView content, has its own `TreeHost` and activation state.
...
`crate::app`'s `WINDOWS` registry (Issue #254) is the application-layer strong lifetime authority for the final most-derived `Rc<dyn WindowExt>` once a `Window` has been shown for the first time — retained via `retain_window` on first `show()` (keyed off `__self_weak`, captured at `Window::construct()`), with the retention id stored on `ElwinduiWindowIvars`. Release happens from a new `ElwinduiWindow::windowWillClose:` delegate method, deliberately distinct from the pre-close veto hook (`windowShouldClose:`, below) — `windowWillClose:` holds a temporary strong native retain across the release call so releasing the Rust owner cannot free `InnerWindow`'s `Retained<NSWindow>` while AppKit is still inside the native close callback. Application termination remains `applicationShouldTerminateAfterLastWindowClosed:`, unaffected by this registry. See `docs/design/runtime/component_lifecycle_design.md` §4j for the full cross-backend invariant and rationale.

`TreeHost` forwards self-drawn mouse press/move/release through the common `PointerDispatcher`; AppKit's drag event overrides preserve delivery during an active press. A self-drawn press also makes the host the window's first responder so Escape reaches the same host during the gesture. Escape, key-window/application deactivation, host suppression, view detachment/window transfer, and hosted-tree replacement/clear cancel the Core gesture before teardown. A weak `AppKitPointerGestureHost` lets common subtree unmount perform the same ordered cancellation without retaining the native view.
