...
Every `TreeHostPanel` owns exactly one permanent, transparent, hit-test-visible native `Rectangle` (`input_surface`), attached once at construction at the z-bottom of `Canvas.Children()` and kept sized to the host's final relayout viewport — including the unconstrained-axis natural-size case (Issue #236). XAML elements created only as render projections of self-drawn nodes remain input-transparent (`IsHitTestVisible(false)`), so over blank/self-drawn area native hit testing falls through them to this dedicated input surface, not directly to the Canvas: `Canvas.Background` is normally `null`, and a `null` Brush is not itself hit-testable.

The root Canvas owns all self-drawn pointer handlers and native pointer capture; the input surface supplies only an eligible `OriginalSource` for Core forwarding and is never a second dispatcher or capture owner. The root Canvas forwards self-drawn pointer press/move/release/canceled events to the common `PointerDispatcher` only when `OriginalSource` is the exact root Canvas or the exact `input_surface`; any other source — in particular a real `NativeControl` child, which remains its own native input owner — is rejected, so a native control's gesture is never duplicated into Core. A successful press captures the native pointer and release relinquishes it. `PointerCanceled` clears and notifies the Core capture before the Canvas releases its native captures; the resulting `PointerCaptureLost`, or an independently initiated capture loss, enters the same idempotent Core cancellation path. A weak `WinUI3PointerGestureHost` applies the Core-first ordering for subtree unmount, host deactivation, and tree replacement/clear. `Window.transparent` toggles only the root Canvas's own visual background and never affects the input surface's attachment or hit-testability.
