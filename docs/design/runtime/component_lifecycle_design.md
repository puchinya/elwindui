...
- This means `elwindui-macros/src/class.rs`'s generated `new()` — today unconditionally `Rc::new_cyclic(construct); __elwindui_run_on_constructed();` — is **in scope for this initiative**. **Correction (post-merge review): this belongs to CI-3, not CI-2.** `#[class]` treats `on_constructed`'s body as opaque tokens (it only detects the `fn on_constructed` item, strips it, and re-emits it verbatim inside the auto-generated `__elwindui_run_on_constructed` chain — see `class.rs`'s `on_constructed_fn` handling) — so CI-2's "split `new()` and the build function, timing unchanged" step can restructure what's *inside* `on_constructed`'s body (make it call a separately named internal build method) without `#[class]` needing to change at all, for both component shapes. `elwindui-macros/src/class.rs` only needs to change once `on_constructed`'s automatic, unconditional invocation itself must become deferrable — that is CI-3's "introduce explicit mount" step, not CI-2's mechanical, behavior-preserving split. CI-2's critical files are `elwindui-codegen` only; CI-3's critical files gain `crates/elwindui-macros/src/class.rs` (see §7).
- The exact mechanism by which `#[class]`'s generated `new()` stops auto-invoking `__elwindui_run_on_constructed()` for `view!`-bearing (mount-needing) classes, while classes with no `view!` (leaf native controls that don't participate in this lifecycle split, data-only classes) may reasonably keep today's automatic behavior, is left to CI-3's own `phase:design` — candidate shapes include a new class-level opt-out attribute (e.g. `#[elwindui::class(deferred_mount)]`), or `generate_view` always supplying its own `mount`-aware `on_constructed` wrapper that the unconditional auto-call still reaches but which no-ops until explicitly re-armed by `mount()`. The opt-out-attribute shape is recommended: `auto_new`'s `__obj.__elwindui_run_on_constructed();` line becomes conditional on the new flag, `__elwindui_run_on_constructed` itself stays `#[doc(hidden)] pub` (already reachable across module/crate boundaries, since base-chaining already requires that), and `generate_view`'s emitted `mount()` calls `self.__elwindui_run_on_constructed()` directly once `__environment` is established. Either shape must preserve `#[class]`'s existing invariants: `construct` remains the single source of constructor arguments, hand-written `new` remains rejected, `on_constructed` chaining remains base-first across `inherits` hops (`macro_class_spec.md` §13.3's "most-derived object" contract for `__self_weak` is unaffected — `mount()` reconstructing a real `Rc<Self>` from the same weak handle is a direct continuation of today's approach, not a replacement).
- For the plain (non-`#[class]`) `has_view == true` shape, CI-2 introduces an equivalent `construct`/mount-body split entirely within `elwindui-codegen` (no `#[class]` involvement, since this shape was never routed through `#[class]` to begin with) so both shapes converge on the same generated method shape post-CI-3.
...
- **CI-2** (#105): no `elwindui-macros` change needed — the `new()`/build split for `#[class]`-composed components stays entirely within `on_constructed`'s (opaque-to-`#[class]`) body, in `elwindui-codegen`. (Earlier revision of this section incorrectly assigned `elwindui-macros/src/class.rs` to CI-2 too; corrected after re-reading `class.rs`'s actual handling of `on_constructed` as opaque tokens.)
- **CI-3** (#106): add `crates/elwindui-macros/src/class.rs` to critical files/areas — this is where `#[class]`-generated `new()`'s unconditional `__elwindui_run_on_constructed()` call must become deferrable. Its design phase decides the exact mechanism (§4 above recommends a class-level opt-out attribute).
...
(Tracking comments recording these amendments, and the CI-2 correction above, are added to #105 and #106.)
