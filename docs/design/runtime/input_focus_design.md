...
AppKit forwards its `NSView` mouse overrides directly and cancels on Escape, window/application deactivation, host detachment, and tree replacement/clear. WinUI3 listens on the root `Canvas`, which owns pointer handlers and native capture; a blank/self-drawn hit resolves to that host's permanent hit-test input surface, not the Canvas's own background (see [`../backends/winui3_backend_design.md`](../backends/winui3_backend_design.md) for construction/sizing detail). Only the exact root `Canvas` or the exact input surface is accepted as an eligible `OriginalSource` — a real native child, or any other XAML descendant, is rejected — and accepted events use native pointer capture so move/release delivery continues outside the Canvas, mapping `PointerCanceled`/`PointerCaptureLost` into the common cancellation path before releasing native capture.
