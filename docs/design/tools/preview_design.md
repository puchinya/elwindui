...
本書は、エディタ内プレビュー機能の設計を定める。ツールチェーン全体のアーキテクチャ概観は `docs/elwindui_tool_codegen_design.md` §6を参照。
...
全体アーキテクチャ図(`docs/elwindui_tool_codegen_design.md`§6)における、プレビューツールの位置づけは「LSPからの出力を受け取り、WebViewへ描画結果を送信する(①②)」経路であり、実行中アプリへの反映(③、ホットリロードツールによるdylib差し替え)へは任意で接続する。
