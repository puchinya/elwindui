...
`HorizontalLayout`。`children: UIElementCollection`(Logicalツリーの子要素リスト、付録H.2.2)は
...
```rust
elwindui_core::ui::new_element(
    elwindui_core::ui::create_vertical_layout( // HorizontalLayoutならcreate_horizontal_layout
        /* spacing属性、省略時は0.0 */,
        elwindui_core::ui::UIElementCollection::new(
...
`elwindui_core::ui::StackImpl`を`base`フィールドとして持ち、`UIElement`をそこへ委譲する
...
```rust
elwindui_core::ui::new_element(elwindui_core::ui::TextBlock {
...
`elwindui_core::ui::new_element(elwindui_core::ui::Shape{..})`を直接組み立てる)
...
付録H.2.2) —— `.elwind`側の`Shape`宣言にも`children`フィールドは存在しない。
...
(付録H.2.2)——`VerticalLayout`/`HorizontalLayout`/`Grid`と違いテンプレート機構を持たないため、
...
`base: elwindui_core::ui::ControlImpl`フィールドを持たせて`elwindui_core::ui::UIElement`と
`elwindui_core::ui::Control`を`self.base`へ委譲する形で直接実装する(docs/elwindui_spec.md
...
`children: UIElementCollection`(Logicalツリーの子要素リスト、付録H.2.2)は共通親`Layout`から
...
- 静的ネストされた`TabViewItem { .. }` — `#[content(children)]`(`children: Vec<TabViewItem>`、WinUI3の`ContentPropertyAttribute`相当、付録H.2.2)で受け取る、タブの集合(`TabViewItem`自体は`UIElement`のVisualツリーに参加しないため`UIElementCollection`ではなく通常の`Vec`のまま——付録X/`MenuBar`/`MenuBarItem`と同じ`{}`ネスト機構)
