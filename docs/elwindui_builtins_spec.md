...
Rust値としてコード生成器が組み立てる。この階層は「どの具象構造体として組み立てられるか」を表す実行時の
分類であり、DSL側の`inherits`(`elwindui_spec.md`§3)とは軸が異なる——`inherits NativeControl`は
今も純粋なカテゴリタグ(フィールド/メソッド継承なし)で、それを宣言する部品(DSLのカテゴリタグ、
ジェネリック引数を持たない)はすべて実ハンドル型`H`でジェネリックなRust構造体`NativeControl<H>`に
なり、専用のネイティブ実体を持たない仮想ビルトインは`Stack`/`Shape`/`TextBlock`/`Control`のいずれかに
なる。一方`inherits`で非`NativeControl`のbuiltin/ユーザーcomponentを継承する場合(`ContentControl
inherits Control`など)は§3の通り**実際のフィールド/`view`テンプレート継承**であり、単なるカテゴリ
タグ付けではない。
...
UIElement (trait, elwindui-core::tree)
 ...
 │   ├─ Dropdown / Option         (付録F.5, 複合型プロパティ Vec<Option> ※.elwind未実装・仕様のみ)
 │   ├─ MenuBar / MenuBarItem     (付録X)
 │   ├─ Menu / MenuItem           (付録M.2)
 │   ├─ TabView / TabViewItem     (付録Y)
 │   ├─ Dialog                    (付録M.1 ※.elwind未実装・仕様のみ)
 │   ├─ Tooltip                   (付録M.3 ※.elwind未実装・仕様のみ)
 │   ├─ NavigationHost            (付録L.2 ※.elwind未実装・仕様のみ)
 │   └─ VirtualList               (付録Q ※.elwind未実装・仕様のみ)
 │
...
「※.elwind未実装・仕様のみ」と付記した部品は、`crates/elwindui-builtins/src/builtins.elwind`に対応する
...
(`HorizontalLayout`も同じ形。実宣言は`elwindui-builtins/src/builtins.elwind`内の`VerticalLayout`/`HorizontalLayout`)
...
```rust
elwindui_core::tree::new_element(elwindui_core::tree::Stack {
    ...
    children: /* 子要素を同じ規則で再帰的にRc<dyn UIElement>化したもの */,
})
```

`new_element`は`Rc::new`した後、各子要素の`UIElementBase.parent`(本物の親ポインタ、付録H参照)を
...
`elwindui-builtins/src/builtins.elwind`内の`TextBlock`):
