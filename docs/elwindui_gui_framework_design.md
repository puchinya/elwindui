...
UIElement (構造体、Margin/Alignment共通実装。baseなしの既定クラス。トレイトは`UIElementExt`。
 ...
 │   │                    `template: Option<ControlTemplate<Self>>`による視覚ツリーの実行時
...
`ElwinduiContentRoot`は目新しい発明ではなく、`TabView`(付録Y)の各タブが既に持つ「タブごとに独立した`TreeHostView`/`TreeHostPanel`インスタンス」という構造の2つ目の適用である——`set_tree`を呼べば、そのホスト自身の`RelayoutHost`/`FocusHost`登録は既存のコード経路がそのまま面倒を見る。新規に必要になった機構は1点のみ:スクロールする軸のMeasureを「利用可能領域にクランプする」のではなく「無制約(`f32::INFINITY`)」で行い、その結果得られた自然サイズへコンテンツホスト自身のフレーム(AppKitの`NSView.frame`)/`Canvas.Width`・`Height`(WinUI3)を明示的に成長させる、という挙動(`elwindui_backend_appkit::inner::TreeHostIvars::unconstrained_axes`/`elwindui_backend_winui3::inner::TreeHostPanel::unconstrained_axes`)。スクロールしない側の軸は、AppKitでは`NSAutoresizingMaskOptions`(`NSNotificationCenter`の購読は不要)、WinUI3では`ScrollViewer.SizeChanged`ハンドラによる明示的な`Width`/`Height`同期(`Canvas`はWinUI3のレイアウトシステムに参加しないため、`TabViewItem.Content`と同じ理由で必要)で、それぞれ外側のビューポートサイズへ追従させる。

`content`内のネイティブコントロール(例: `ScrollView`の中にネストした`TextBox`)へのフォーカスも追加コードなしでカバーされる——§5.5のネイティブフォーカス橋渡しは「任意の`TreeHostView`祖先」を探す設計になっているため、ネストしたコンテンツホストも自動的に対象に入る。
...
```rust
// elwindui-core::focus
pub struct FocusTracker { /* focused element + trap scope stack */ }
...
pub enum FocusDirection { Next, Previous, Up, Down, Left, Right } // Up/Down/Left/Rightは未実装(下記)
```

`FocusTracker`は`elwindui_core::input::KeyboardDispatcher`(§8.1)が1ホストツリーにつき1インスタンス所有する——`PointerDispatcher`と同じ「ホストごとに1つ」というパターン。Tab移動順序は`UIElementExt::tab_stop`/`focus_order`(下記)を`visual_children()`で深さ優先に辿って計算する(`#[focus(order:)]`という専用DSL属性は無い——後述)。`push_trap`/`pop_trap`のAPIは用意済みだが、これを呼ぶ`Dialog`(§8.3)自体が未実装のため今回は呼び出し元が無い。方向キー(`Up`/`Down`/`Left`/`Right`)によるフォーカス移動は各要素の配置矩形を使った2D空間探索が必要で未実装(`move_focus`は`Next`/`Previous`のみ対応)。

**タブストップ/タブ順序は`#[focus(...)]`という専用属性ではなく、`margin`/`hit_test_visible`と同じ普通の共通プロパティ**(`tab_stop: Option<bool>`/`focus_order: Option<i32>`、`component UIElement`に宣言)として実装した——`UIElementExt::set_tab_stop`/`set_focus_order`という単純な構築後セッターで十分表現できるため、新しいDSL属性構文もパーサー変更も不要になる:
...
ネイティブリーフ(`Button`/`TextArea`/`TabView`)は各バックエンドの`new()`が`set_tab_stop(true)`を既定で呼ぶ(WinUI3の`Control.IsTabStop`既定値に相当)。自前描画系要素(`Layout`/`Control`/`Shape`/`TextBlock`)は既定`false`で、明示的に`tab_stop: true`を書いた場合のみタブ順序に参加する。

**フォーカス状態(WinUI3の`FocusState`)**:「今フォーカスがあるか」に加え「どうやってフォーカスを得たか」まで`UIElement`自身が実プロパティとして持つ:
...
// elwindui-core::ui
pub trait FocusHost {
    fn request_focus(&self, target: &Rc<dyn UIElementExt>) -> bool; // 常に FocusState::Programmatic
}
```
...
**広域既定値(WinUI3の`Style`相当)は`store`を流用する**——詳細は§7.1参照。
...
**実装状況**: 実装済み(AppKit・WinUI3両バックエンドに実配線。WinUI3側はWindows環境が無く未検証)。`elwindui-codegen`は`#[shortcut(...)]`のみ新規パース対象——`on_key_down`/`on_key_up`/`on_text_input`は`component UIElement`が宣言する`#[routed]`フィールド(§5.10の9個のポインタイベントと同じ仕組み)として実装されており、新しい構文もコード生成の特別扱いも不要だった。`#[focus(order/trap)]`は§5.5の通り専用属性としては実装せず、`tab_stop`/`focus_order`という普通の共通プロパティに置き換えた。
...
- `on_key_down` / `on_key_up` — 物理キーの押下・離上(`elwindui_core::input::KeyEventArgs { key: Key, modifiers: KeyModifiers, is_repeat: bool }`を受け取る)。バブリング(`dispatch_routed`)——ヒットテストではなく`FocusTracker::focused()`が返す要素が起点。
- `on_text_input` — IME確定後の実文字列、または直接入力の文字を受け取る(`elwindui_core::input::TextInputEventArgs { text: String }`)。**実装範囲の制限**: AppKit側は`NSTextInputClient`のフル実装(IME変換中プレビュー表示)ではなく、`keyDown:`の`characters()`をそのまま渡す簡易実装——直接入力(ローマ字非経由)のみ確実に動作する。WinUI3側は`UIElement.CharacterReceived`(確定済み文字のみ配送)を使うため、この制限は無い。
- `on_got_focus` / `on_lost_focus` — 非バブリング(`dispatch_direct`、WinUI3の`GotFocus`/`LostFocus`と同じ)。`FocusTracker::set_focus`/`clear_focus`が発火する(§5.5)。
...
既定では、`#[shortcut(...)]`が付いた要素はその要素がフォーカスされていなくてもアプリウィンドウ内であれば発火する(メニューショートカットと同じ扱い、`ShortcutScope::Global`)。要素にフォーカスがある場合のみ発火させたい場合は`scope: local`を指定する:
