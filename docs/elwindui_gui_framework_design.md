...
- View側はViewModelを`#[param] #[inject]`(実体は`#[bindable]`、下記)で受け取り(§7.1の`#[scoped]`+`#[inject]`と同じ注入パターン)、双方向編集フィールドは`bind!(vm.field, TwoWay)`でpropに写し取り、読み取り専用表示(`vm.window_title`等)は`view`式中で直接参照してよい(14章ルール13の対象外 — ルール13は`#[param]`初期化式への直接参照のみを禁止)
- `viewmodel`は`.elwind`のDSLネイティブ構文として書く以外に、通常のRustファイルに`#[elwindui::viewmodel] mod foo { struct Foo { ... } impl Foo { async fn save(&self) { .. } } }`として書くこともできる(WPF/WinUI3のMVVMがViewModelをホスト言語側に置くのと同様)。どちらの書き方でも、View側は§2.10の規則どおり実際のRustパスを`use`する

**`command`属性(WinUI3の`Command`プロパティ相当の糖衣構文)**: `on_click: vm.save.execute()` + `enabled: vm.save.can_execute`という2属性を、`command: vm.save`の1属性にまとめて書ける。対象の`component`/ビルトインが自前で宣言している唯一の`on_*`フィールド(`Button`なら`on_click`)へ自動配線し、`enabled`フィールドを持つ対象なら`can_execute`への結線も追加する。`on_*`フィールドが0個または複数ある場合は展開されない。明示的な`on_click`/`enabled`と併記した場合は明示指定が優先される(`command`は未設定の属性を補うだけ)。

**再同期のタイミング**: コード生成器は初期構築時だけ全属性を設定する`resync()`を生成し、以後の更新はブランケットな再同期ではなく各`viewmodel`の型付き`PropertyChanged`購読で行う。`#[observable]`のsetterは代入後に対応する`PropertyId`を通知し、`#[computed]`/`can_execute`は依存するsetterの後で再計算されて自身の`PropertyId`も通知する。`Subscription`は表示オブジェクトが保持し、破棄時にDropで解除される。子viewmodelの変更を親viewmodelのコレクション変更として転送しない(文書本文の変更は`TextArea`と文字数表示だけを更新し、親`TabView`のchildrenは更新しない)。

**低オーバーヘッドな内部表現**: 依存関係はコンパイル時に静的抽出し(`#[computed]`と同一の仕組み)、動的な購読リスト(`Vec<Box<dyn Fn()>>`)は持たない。`Copy`可能な型は`Cell<T>`、非`Copy`型のみ`RefCell`で保持し、`Command`の本体は具体的なクロージャ型として単相化する(`dyn Trait`を使わない)。複雑な相互依存で静的解析が困難な場合のみ、`elwindui-core`が提供する汎用リアクティブグラフ(スロットマップ+世代インデックスの`SignalId`、Leptos/Xilem系のリアクティブランタイムと同様の設計)にフォールバックする。
...
ファイル読込・API呼び出し等の非同期処理と、`prop`/`Command`の連携を定義する。新しい実行モデルは導入せず、既存の`#[computed]`・`Command`を非同期版に拡張する。
...
`#[undoable]`フィールドが1つ以上ある`viewmodel`には、`undo: Command`/`redo: Command`と対応する`can_undo`/`can_redo`(§7.2の`Command`と同じ仕組み)が自動的に追加され、§7.2の通常のCommandと同じ書き方で結線できる:
...
**実装状況**: `ErrorBoundary`ビルトイン・`#[command(catches: ...)]`ともに`crates/elwindui-codegen/src/builtins.elwind`にはまだ定義がなく未実装。本節は設計のみ。
