...
| `elwindui-languageserver` | 約900行 | 部分実装。2026-08-01に`.elwind`ディレクトリ単位のモデルから単一`.rs`ファイル単位のモデルへ retarget(`component_frontend::modules_from_file`が`#[elwindui::component]`/`#[elwindui::viewmodel]`/`#[elwindui::dsl_enum]`を実マクロ展開せずに読み出す)。診断(`elwindui-codegen`の`component_frontend`/`validate`を再利用、`syn::parse_file`失敗時は実際の行・列付き)、メンバー補完(`vm.field`)が実働。シンタックスハイライト(semantic tokens)は当初`.elwind`専用トークナイザを実装していたが対象が実Rustファイルになったため一旦削除(rust-analyzer自身のハイライトで足りる)、その後Issue #14の未解決の論点を解消する形で`view! { .. }`マクロ本体だけに限定して復活済み(`src/semantic_tokens.rs`、`proc-macro2`の`span-locations`機能で実ソース上のバイト範囲を特定し、その範囲内だけトークナイズ——rust-analyzer自身のRustハイライトとは二重提供にならない)。旧モデルにあったディレクトリ横断のクロスファイル解決(`bind!`/`vm.field`を別ファイルのviewmodelと突き合わせる)は無い ── 実マクロ展開経路自体、宣言順に依存する同一クレート内レジストリにしか頼っておらず、実コンパイルを行わないLSPには再現不能なため。hover、生成コードプレビュー、オフスクリーンレンダリングと連携したインスタンス生成パイプラインは未実装 |
...
| `elwindui-languageserver`(LSP) | 部分実装。単一`.rs`ファイル単位の診断・メンバー補完まで実働(2026-08-01 retarget、§1参照)。シンタックスハイライトは`view! { .. }`マクロ本体に限定して実装済み(rust-analyzer自身のRustハイライトとは非重複)。hover・プレビュー用インスタンス生成パイプラインは未実装 |
