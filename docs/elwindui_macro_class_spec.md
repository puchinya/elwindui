...
`UIElement`のみが1つの共有`__elwindui_inherit_terminal!`を`elwindui-core`に生成し、他の全クラスが
それを(クレートを跨いで)自己参照する方式は採用していない——この共有マクロへの参照パスは
「マクロを定義したクレート内での`Self`解決」に依存しており、**3クレート以上の連鎖**(例:
バックエンドクレートの`Window`が、消費者クレートの`inherits = ..`チェーンの中間に現れる場合)で
解決に失敗するため。かわりに各クラスが自分専用のローカルな終端チェックを持つ——`$crate::`は
常に「このマクロを定義したクレート」を指すため、クレート数に関係なく確実に解決する。
...
## 13. コンストラクタ自動生成(`construct` → `new`)、`__self_weak`、`on_constructed`

### 13.1 `construct`は必須

`impl ClassName { .. }`には、`&self`を持たない(レシーバ無し)、戻り値が厳密に`Self`の
`construct`関数がちょうど1つ必要——0個(未定義)でも2個以上(重複定義)でもコンパイルエラーになる。
暗黙のデフォルト`construct`は一切合成されない。`trait_only`(裏付けのstructを持たない)クラスは
この要件の対象外。

```rust
#[elwindui_macros::class]
impl Button {
    fn construct(text: String) -> Self {
        Self { base: NativeControl::construct(handle), text }
    }
}
```
...
`#[class]`は`construct`から`pub fn new(..) -> std::rc::Rc<Self>`を常に自動生成する
(下記13.3)。`impl`内に`fn new`という名前のメソッドを書くと、レシーバの有無やシグネチャに
関わらずコンパイルエラーになる——`Rc::new`後に必要だった後処理(親ポインタ配線・イベント
配線・初回`resync()`呼び出し等)は、全て13.4の`on_constructed`に書く。

例外は`abstract_class`のみ:`construct`は定義できる(具象サブクラスが自分の`base`フィールドを
組み立てる際に呼ぶための土台、例: `Layout::construct()`)が、`new`は自動生成されない
(直接インスタンス化されることを想定しないクラスのため)。

### 13.3 `__self_weak`と自動生成される`new`

`construct`の本体内では、特殊識別子`__self_weak`を使って、`Rc::new_cyclic`で構築中の
最終派生オブジェクトへの弱参照を取得できる:
...
マクロは`construct`を内部で次のように変換する(利用者が直接目にすることはない、
`#[doc(hidden)] pub fn`):

- 関数名を`__class_construct`にリネームする。
- 先頭に隠しパラメータ`__class_self_weak: std::rc::Weak<dyn {このクラス自身のExtトレイト}>`
  を追加する——**ルート型の伝播は一切行わない**。「このクラス自身のExtトレイト」は、通常/
  ルートクラスなら`{ClassName}Ext`、`struct_only`クラスならその`struct_only`先のトレイトで、
  常にこのクラス自身の情報だけから自己完結で決まる。
- 本体内の`__self_weak`への参照を全て`__class_self_weak`に書き換える(`syn::visit_mut`に
  よるAST書き換え、文字列置換ではない)。
- 本体内で、このクラス自身の`inherits = Parent`先の`Parent::construct(..)`という形の呼び出し
  (無関係な`Type::construct()`呼び出しは対象外)を、`Parent::__class_construct(
  __class_self_weak.clone(), ..)`に書き換える。型注釈やキャストは不要——`Parent::
  __class_construct`自身のパラメータ型がそのまま呼び出し引数位置での暗黙のtrait upcasting
  coercionのターゲットになる。これは、このマクロが既に生成している`{Child}Ext: {Parent}Ext`
  というスーパートレイト境界により、`Weak<dyn ChildExt>`から`Weak<dyn ParentExt>`への変換が
  常に(クレート境界を跨いでいても)自動的に成立するために可能になっている
  (`Weak<T>: CoerceUnsized<Weak<U>> where T: Unsize<U>`)。

自動生成される`new`は`Rc::new_cyclic`を使う:
...
pub fn new(padding: Option<f32>) -> std::rc::Rc<Self> {
    let __obj = std::rc::Rc::new_cyclic(|__weak_self: &std::rc::Weak<Self>| {
        Self::__class_construct(__weak_self.clone(), padding)
    });
...
`__weak_self.clone()`(型`Weak<Self>`)を`Self::__class_construct`の第一引数の位置に渡す
だけで、`impl {OwnExt} for Self`が既に存在すること(`Self: {OwnExt}`)により暗黙の unsized
coercion が働く——ここでも型注釈は不要。`Rc::new_cyclic`のクロージャ実行中(つまり
`construct`本体の実行中)は`__self_weak.upgrade()`は常に`None`を返す(`Rc::new_cyclic`
自体の制約)。`__self_weak`は次の用途にのみ使うこと:フィールドへ保存する、後で呼ばれる
クロージャへキャプチャする、親クラスへ渡す。構築中に`upgrade()`を成功させるための二段階
初期化・`unsafe`・`OnceCell`/`RefCell<Option<_>>`は導入しないこと。

`__self_weak`という名前は予約されており、`construct`のパラメータ名や`let`束縛として使うと
コンパイルエラーになる。`construct`関数の外(通常のメソッド等)での使用は、単に未定義識別子
エラーになる。

### 13.4 `on_constructed`

`construct`が定義されているクラスは、任意で`fn on_constructed(&self)`を定義できる。
定義すると、`Rc::new_cyclic`が完了した直後(`Rc`が実在する状態)に、マクロが自動的に
ちょうど一度だけ実行する——利用者が手動で呼び出す必要はない。
...
impl Button {
    ...
    fn on_constructed(&self) {
        // ここでは Rc::new_cyclic が完了している。
        // construct中に保存した __self_weak の upgrade() は成功し得る。
        self.set_tab_stop(true);
    }
}
```

- シグネチャは厳密に`fn on_constructed(&self)`——`&mut self`・追加引数・`async`・
  `()`以外の戻り値は全てコンパイルエラー。
- `on_constructed`は`{ClassName}Ext`トレイトのメソッドには**ならない**(動的ディスパッチは
  不要——`Rc::new_cyclic`直後に、構築対象の具象型に対して一度だけ呼ばれるだけなので)。
  通常のプレーンな(inherentな)メソッドとして生成される。
- 継承チェーンの各クラスが独自に`on_constructed`を定義でき、マクロが生成する隠しメソッド
  `#[doc(hidden)] pub fn __elwindui_run_on_constructed(&self)`が、**基底クラスから
  最派生クラスの順に**、チェーン全体を自動的に連鎖実行する:

```rust
// 概念的な展開(Button inherits NativeControl inherits UIElement の場合)
fn __elwindui_run_on_constructed(&self) {  // Button自身
    self.base.__elwindui_run_on_constructed();  // NativeControl → UIElement の順に先に実行
    self.on_constructed();  // 定義していれば
}
```

派生クラスの`on_constructed`から親クラスの`on_constructed`を手動で呼び出してはならない
(呼び出しはマクロ生成コードが担当する——二重実行の原因になる)。

自分自身の`construct`から呼ばれる`Parent::construct(..)`(13.3の書き換え)は、
「親クラスの`base`フィールドを構築する責任」を表すのに対し、`__elwindui_run_on_constructed`
の連鎖は「`Rc`完成後の後処理を基底から実行する責任」を表す——役割が明確に分かれている
(前者は各クラスの`construct`本体が手書きする、後者はマクロが自動生成する)。
...
**一切変更されておらず、`cfg`で隠されてもいない**。成功ケースも含めて実生成物全体を
`#[cfg(not(rust_analyzer))]`で隠し、rust-analyzerには常にこのshadowだけを見せる設計は
採用していない。理由:
...
1. **`#[cfg(...)]`は直後の1アイテムにしか掛からない**。`trait_decl`/`trait_impl`/`ctor_block`/
   ...
   無条件にコンパイルされてしまう(`shadow`と衝突し、`E0034`等で実ビルド自体が壊れる)。
   `mod { .. }`でラップしてアイテムひとつに畳む案も、`inherit_macro_path`(§8.2)が
   同一クレート内の祖先マクロを**裸名**で呼ぶ設計(`$crate::`やパス修飾を使わない)に依存して
   おり、生成物を1段深いモジュールへ移すとその裸名解決が壊れる(`cannot find macro ... in this
   scope`)ため成立しない。裸名呼び出し規約自体を変える(=より大きな設計変更)よりも、そもそも
   成功ケースを隠す必要性自体を見直す方を選んでいる。
