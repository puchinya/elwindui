...
**Phase 1の実装自体は完了**(§1〜§1.8): 共通基盤のフォーカス配線、TextBox、PasswordBox、ScrollView、デモアプリのすべてがAppKit上で`cargo build`/`cargo test`通過、WinUI3へ構造ミラー済み。**未完了なのは対話的な目視確認(クリック・入力・フォーカス切り替え・スクロール)のみ**——このマシンの実行環境に画面収録・アクセシビリティ権限が付与されていないため、ユーザーによる手動確認が必要(§1・§1.8参照)。GTK4は上記の通り意図的にスコープ外。
