...
- `template: template_view!(|alias: Self| { ... })`を宣言するControl-derived componentは、同じmount-time contextから型付き`Option<ControlTemplate<Component>>`を一度だけ解決する。Key変更はmount済みControlを再テンプレート化しない。詳細は[`control_template_spec.md`](control_template_spec.md)を参照する。
