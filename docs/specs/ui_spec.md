...
| `context_popup` | `Option<ViewFactory>` | OneWay | 任意のUIElementツリーを内容とするCustom Context Popup（既定値: `None`）。`ViewFactory` は popup 専用ではない汎用の deferred View factory型 — 詳細は `docs/design/runtime/view_factory_design.md` |
...
- **Lifecycle & Environment**:
  - `context_popup` の内容は表示要求（open）時点で構築（build）される。owner Component の mount 時点で一度だけ構築されるのではなく、popup が開かれるたびに新たに `ViewFactory::build` が呼ばれる。
  - `ViewFactory` の factory は owner を `Weak` としてのみ捕捉する（strong retain cycle を作らない）。owner が既に解放されている場合、`ViewFactory::build` は `None` を返し、popup は表示されない。
  - **契約の区別**: 上記は `ViewFactory` 自体（低レベル API、`ViewFactory::new(|ctx| ...)`）が機械的に保証する範囲である。「owner の現在値を自動的に読む」こと自体は、この低レベル API 単体では保証されない——`ctx.owner`/`ctx.environment` から現在値を読むかどうかはクロージャの書き方次第であり、誤って構築前の値をキャプチャすることも可能である。「owner の bare な識別子参照が自動的に現在値を読む」という保証は、宣言的 `context_popup: view! { .. }` DSL（下記、実装済み）が提供する、より強い契約である。詳細は [`../design/runtime/view_factory_design.md`](../design/runtime/view_factory_design.md) §4 を参照。
  ...
  - **Build abort（owner 消失）**: `ViewFactory::build` は owner が既に解放されている場合 `None` を返す（この判定は `ViewFactory` 自体が機械的に強制する——factory クロージャが `ctx.owner` を一度も参照しなくても、owner が死んでいれば factory 自体が呼ばれない）。`ContextMenuService::open_custom_popup` はこの場合 popup を一切表示せず `None` を返す。
...
DSL/codegenの内部ではControlのscalar `visual_root` content destinationも定義されるが、これは
旧body composition用の公開slotではなく、`template_root`のprivate ownershipを二重化しないための
internal metadata surfaceである。
