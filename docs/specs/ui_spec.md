...
- **Lifecycle & Environment**:
  ...
  - **Close 時の解放**: ポップアップが閉じられた際（外部クリック、Escape キー、項目選択、`PopupDismissAction`、popup replacement、owner Window の close 等）は、`elwindui_core::ui::unmount_subtree` による child-first の再帰的 unmount（`on_unmount`・購読解除を含む）が、バックエンドのネイティブ detach（ウィンドウ関係の解除・可視性変更・ホストツリーの解放を含む、そのいずれよりも前）に必ず先行して実行される（teardown-before-detach）。さらに、close 完了後は `PopupSurfaceHandle` 自身が popup content root への強参照を保持し続けてはならない——unmount 済みだが解放されない、という状態を作らない（`InnerPopupSurface` の `content` フィールドは `RefCell<Option<Rc<..>>>` とし、`close()` 内の `take()` で解放する）。
