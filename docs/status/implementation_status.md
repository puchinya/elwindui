...
| Accessibility | 🚧 | Core-owned semantic snapshots, stable `AccessibilityId`, ordinary UIElement properties/hooks, intrinsic built-in mappings, disabled-state synchronization, template-aware Automatic traversal, inactive-host suppression, AppKit synthetic AX projection, and the shared AppKit E2E baseline are implemented. The WinUI XAML AutomationPeer bridge now includes Core-gated UIA patterns, AutomationId, Focus dispatch, physical screen bounds, deployed metadata, and verified external Invoke/Toggle/RangeValue/Value/Focus actions; full shared-scenario evidence remains partial under [#256](https://github.com/puchinya/elwindui/issues/256) because visual completion capture is unavailable and the optional slider step action was not exercised. |
...
- Accessibility is adopted as a Core-owned semantic tree through child implementation Issue [#253](https://github.com/puchinya/elwindui/issues/253). AppKit shared E2E passes twice with the checked-in driver, including direct AX TextArea SetText; WinUI's Core-gated pattern projection, identity, physical bounds, duplicate suppression, external core-backed action paths, logical exit removal, stale-action rejection, and teardown are implemented, while the remaining full-scenario visual/optional-step evidence is tracked under [#256](https://github.com/puchinya/elwindui/issues/256). NavigationHost, VirtualList, and ErrorBoundary remain unresolved under decision Issue [#85](https://github.com/puchinya/elwindui/issues/85).
