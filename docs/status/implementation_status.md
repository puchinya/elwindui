...
- Accessibility is adopted as a Core-owned semantic tree through child implementation Issue [#253](https://github.com/puchinya/elwindui/issues/253). AppKit shared E2E passes twice with the checked-in driver, including direct AX TextArea SetText; WinUI's Core-gated pattern projection, identity, physical bounds, duplicate suppression, external core-backed action paths, logical exit removal, stale-action rejection, and teardown are implemented and the full shared Windows accessibility scenario passed twice on the merged #265 baseline. Slider Increment/Decrement is N/A for the current fixture because it advertises no concrete step; SelectionItem and ExpandCollapse remain implemented without live shared-fixture nodes.
