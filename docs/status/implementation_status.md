...
- Accessibility is adopted as a Core-owned semantic tree through child implementation Issue [#253](https://github.com/puchinya/elwindui/issues/253). AppKit shared E2E passes twice with the checked-in driver, including direct AX TextArea SetText; WinUI's Core-gated pattern projection, identity, physical bounds, duplicate suppression, and teardown are implemented, and the generated metadata is deployed beside the tested executables, while external provider actions remain blocked at the WinUI UIA bridge boundary under [#256](https://github.com/puchinya/elwindui/issues/256). NavigationHost, VirtualList, and ErrorBoundary remain unresolved under decision Issue [#85](https://github.com/puchinya/elwindui/issues/85).
