...
| Animation and transitions | 🚧 | Core target/presentation animation, transactions, springs, transform-aware hit testing, dynamic insertion/removal transitions, AppKit `CVDisplayLink`, WinUI 3 `CompositionTarget.Rendering` projection, and the `animation-demo` harness are implemented. AppKit evidence and final WinUI 3 runtime evidence for Issue #248 are available on Rust HEAD `d8633ce5d0e8bfa6476687ebef599128cf631b26`; GTK4 remains unverified. |
| Accessibility | 🚧 | Core-owned semantic snapshots, stable `AccessibilityId`, ordinary UIElement properties/hooks, intrinsic built-in mappings, disabled-state synchronization, template-aware Automatic traversal, inactive-host suppression, AppKit synthetic AX projection, and the shared AppKit E2E baseline are implemented. The WinUI XAML AutomationPeer bridge now includes Core-gated UIA patterns, AutomationId, Focus dispatch, physical screen bounds, deployed metadata, duplicate suppression, logical removal, stale-action rejection, and teardown. The full shared Windows accessibility scenario passed twice on the merged #265 baseline with valid screen-capture fallback evidence after black WGC captures. Slider Increment/Decrement remains N/A because the fixture advertises no concrete step; SelectionItem and ExpandCollapse are implemented but have no live shared-fixture nodes. |
...
- Accessibility is adopted as a Core-owned semantic tree through child implementation Issue [#253](https://github.com/puchinya/elwindui/issues/253). AppKit shared E2E passes twice with the checked-in driver, including direct AX TextArea SetText; WinUI's Core-gated pattern projection, identity, physical bounds, duplicate suppression, external core-backed action paths, logical exit removal, stale-action rejection, and teardown are implemented and the full shared Windows accessibility scenario passed twice on the merged #265 baseline. Slider Increment/Decrement is N/A for the current fixture because it advertises no concrete step; SelectionItem and ExpandCollapse remain implemented without live shared-fixture nodes. NavigationHost, VirtualList, and ErrorBoundary remain unresolved under decision Issue [#85](https://github.com/puchinya/elwindui/issues/85).
