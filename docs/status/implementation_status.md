...
| 属性代入(`:` / `<=>`)とcomponent state | ✅ `ViewAttribute`の型付き`AssignmentKind`、依存解析による暗黙Once/OneWay、明示TwoWay、`#[state(default = ...)]`を実装。安定Rc identityを持つ`for` itemの直接可変fieldにもTwoWay対応 |
