...
|---|---|---|
| `elwindui-codegen` | 🚧 | component/ViewModel/enum/ControlTemplate frontend, parser, diagnostics, binding and dynamic-region code generation; `body: view!` remains ordinary composition, while `template: template_view!` compiles to a typed `ControlTemplate<C>` and bare children lower from effective `#[content(field)]` metadata plus field type; Control-specific type-name lowering and hidden body-presentation metadata are absent. Standalone typed `templated_parent` inference is tracked separately in [#189](https://github.com/puchinya/elwindui/issues/189) |
