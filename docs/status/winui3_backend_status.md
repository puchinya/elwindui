...
**This is a load-bearing architectural decision — read this section
...
- **Why a C++ shim rather than pure Rust**: this
...
### `notepad` File>Open reentrancy panic, menu-bar/`TabView` content sizing
...
- **Why**: reading the actual generated code (`target/debug/build/notepad-*/out/notepad_window.rs`),
  every command handler generated from `notepad_window.rs` (`on_select` for New/Open/Save, the
  ...
  - `elwindui-codegen`'s `collection_uses_rc_identity` (`codegen.rs`) is supposed to pick
    ...
    the module the `for` loop's own file (`notepad_window.rs`) parses into. **`compile_dir_with_
    ...
    it does, and `notepad_window.rs`'s own `use crate::notepad_view_model::NotepadViewModel;`
    resolves `vm`'s own type fine. The actual failure is one level down: `notepad_window.rs`
    never names `DocumentViewModel` directly (`doc` is only ever the `for` loop's implicit binding),
    so it has no `use` for it, while `document_view.rs` (which declares `DocumentView.doc: Rc
  ...
  - This was **not specific to `notepad`, to `TabView`, or to WinUI3**: any DSL `for` loop over
    a `#[bindable] Rc<SomeProcMacroViewModel>` field's `Vec<...>` collection — i.e. the standard,
    documented way to combine `#[elwindui::viewmodel]` with a view — hit this same
...
- **Fixed** via a different approach than resolve-scope patching (user's own design,
  strictly better — see `docs/design/gui_framework_design.md` §7.2's `#[bindable]`): rather than
  ...
  via same-directory visibility or `sibling_component_modules`) — never the viewmodel type
...
- ~~Fix `elwindui-codegen`'s `for`-loop identity detection~~ — **done**, see "Root cause
...
### `TabView` tab content painting and the design rule behind it
...
File>New-then-back-to-tab-1 regression check (dialog-free verification method) still passes — the diff-based native-children reconciliation didn't regress dynamic content
updates.

### `graphics-demo`'s Compositing/SVG native crash
...
### `graphics-demo`'s Images tab clipping (an application-level bug, not a backend bug)
...
- **Deferred from the same investigation as the `Width`/`Height`-reset fix above — each
  individually reasoned through and cross-referenced against source, but not implemented or
  tested:**
...
- Continue a requirement-by-requirement audit of the backend against `elwindui-core` traits and the
  AppKit implementation. Search for semantic gaps, not only TODO comments — the known
...
- Do not automatically format the whole checkout: it may have unrelated, user-owned dirty files.
