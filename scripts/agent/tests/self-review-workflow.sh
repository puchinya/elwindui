...
if "%~1 %~2"=="issue view" (
  type "%SELF_REVIEW_TEST_ROOT%\.agent-state\issue.json"
  exit /b 0
)
...
canonical_checkbox_contract=$'transport heading\nELWINDUI_REVIEWER_CHECKLIST_V1_BEGIN\n- [ ] checkbox obligation one\n* [x] checkbox obligation two\nELWINDUI_REVIEWER_CHECKLIST_V1_END\n'
write_contract "$canonical_checkbox_contract"
out="$($TMP/scripts/agent/prepare-self-review.sh 123)"
assert_contains "$out" 'items=2'
checkbox_sha="$(sed -n 's/^review_checklist_sha256=//p' <<<"$out")"
grep -q -- '- C001 | contract | checkbox obligation one' .agent-state/issues/123/reviewer-checklist.md
grep -q -- '- C002 | contract | checkbox obligation two' .agent-state/issues/123/reviewer-checklist.md
if command -v pwsh >/dev/null 2>&1; then
  ps_out="$(pwsh -NoProfile -File "$TMP/scripts/agent/prepare-self-review.ps1" 123)"
  [[ "$(sed -n 's/^review_checklist_sha256=//p' <<<"$ps_out")" == "$checkbox_sha" ]]
  "$TMP/scripts/agent/validate-self-review.sh" 123 >/dev/null 2>&1 || true
  fill_pass_review
  validate_out="$($TMP/scripts/agent/validate-self-review.sh 123)"
  assert_contains "$validate_out" 'self_review_status=pass'
  pwsh_validate_out="$(pwsh -NoProfile -File "$TMP/scripts/agent/validate-self-review.ps1" 123)"
  assert_contains "$pwsh_validate_out" 'self_review_status=pass'
  echo 'T33 canonical Markdown checkbox POSIX/PowerShell prepare/validate parity: PASS'
else
  echo 'T33 canonical Markdown checkbox POSIX/PowerShell parity: NOT RUN (pwsh unavailable)'
fi
...
expect_canonical_failure canonical-checklist-empty $'ELWINDUI_REVIEWER_CHECKLIST_V1_BEGIN\n- [ ]\nELWINDUI_REVIEWER_CHECKLIST_V1_END'
echo 'T29b empty canonical checkbox item: PASS'
expect_canonical_failure canonical-checklist-malformed $'ELWINDUI_REVIEWER_CHECKLIST_V1_BEGIN\nunexpected line\nELWINDUI_REVIEWER_CHECKLIST_V1_END'
